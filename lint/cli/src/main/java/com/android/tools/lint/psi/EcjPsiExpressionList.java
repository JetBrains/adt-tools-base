...
class EcjPsiExpressionList extends EcjPsiSourceElement implements PsiExpressionList {
    ...
    EcjPsiExpressionList(@NonNull EcjPsiManager manager) {
        super(manager, null);
    }
    ...
    void setExpressions(PsiExpression[] expressions) {
    ...
    public PsiType[] getExpressionTypes() {
        List<PsiType> types = Lists.newArrayListWithCapacity(mExpressions.length);
        for (PsiExpression expression : mExpressions) {

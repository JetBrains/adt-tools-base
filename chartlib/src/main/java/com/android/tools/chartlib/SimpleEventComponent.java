...
public class SimpleEventComponent<E extends Enum<E>> extends AnimatedComponent {
    ...
    @NonNull
    private final ArrayList<Pair<Integer, Long>> mIconsToDraw;
    ...
    public SimpleEventComponent(
        ...
        mIconsToDraw = new ArrayList<Pair<Integer, Long>>();
    ...
    protected void updateData() {
        ...
        for (int i = 0; i < mData.getSeries().size(); i++) {
            ...
            if (data.getValue() == Action.DOWN) {
                ...
                mIconsToDraw.add(new Pair<Integer, Long>(toDraw, data.getEnd()));
                downEvent = null;
            }
        }
        if (downEvent != null) {
            ...
            if (now - downEvent.getStart() >= HOLD_DELAY_MS) {
                ...
                path.lineTo(end, LINE_OFFSET);
                mPaths.add(path);
                mIconsToDraw.add(new Pair<Integer, Long>(toDraw, downEvent.getEnd()));
            }
        }
    }
    ...
    protected void draw(Graphics2D g2d) {
        ...
        for (int i = 0; i < mIconsToDraw.size(); i++) {
            Pair<Integer, Long> data = mIconsToDraw.get(i);
            double normalizedPosition = ((data.getValue() - min) / (max - min));
            if (data.getValue() == 0) {
            ...
            g2d.drawImage(mIcons[data.getKey() % mIcons.length], translate, null);
        }
